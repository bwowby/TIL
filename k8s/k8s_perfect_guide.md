...
물리 머신에 로컬 k8s 쉽게 구축,실행할 수 있는 도구. 

네트워크가 연결되지 않아도 바로 눈으로 확인 가능하여 개인적 테스트 및 개발 환경으로 적당.

물리 머신에서 손쉽게 k8s 테스트할 수 있는 방법. 한대의 머신에 올인원으로 구성 → 이중화가 보장되지 않음 / 일부 사용할 수 없는 기능도 있음

### 3.2.1 minikube

- **단일 노드 구성**이라 여러대 구성이 필요한 k8s 기능 사용 불가
- 하이퍼바이저 필요 : 하이퍼바이저에 맞는 드라이버를 사용해 조작함으로 호스트가 되는 컨테이너/가상 머신을 생성 후 그 환경에 쿠버네티스 설치 (미니큐브 클러스터)
- kubectl 로 조작
...
$ minikube start driver=hyperkit --kubernetes-versionv1.18.15

$ minikube stats # 상태 확인

$ kubectl config use-context minikube 
	# 여러 클러스터 사용하는 경우 컨텍스트 전환
	# kubectl은 미니큐브를 k8s 노드로 인식
...
### 3.2.2 docker desktop

### 3.2.3 kind

도커 컨테이너를 여러 개 기동 후 그 컨테이너를 쿠버네티스 노드로 사용하여 여러대로 구성된 클러스터를 구축.

로컬 환경에서 멀티노드 클러스터 구축 시 kind 사용이 가장 좋음.
